...
- `docs/design_theory.md` —— **早一版（v2 跟踪律）的**响应环流体模型分析与整定流程，已被 `design_theory_v4.md` 取代，保留是因为论文素材仍引用它。原说明如下：响应环的流体模型分析，以及**整定流程**：均衡、收敛、稳定性的推导，再反解成一套顺序步骤，回答「换一个拓扑该怎么把参数算出来」（第三部分）。design.md §3 的 $V$ 与 §5.3 的 $k$、$\zeta$ 从哪来，看这里。
