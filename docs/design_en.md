...
The control skeleton has a clear lineage: edge arbitration, receiver-side measurement with sender-side enforcement, explicit feedback, and headroom all trace back to EyeQ and the edge bandwidth-guarantee systems around it; manufacturing a signal with a virtual queue comes from HULL's phantom queue and AVQ's integral marking. The skeleton is mature and we use it as it is.
...
**The upper bound on $k$ comes from the target's staleness.** Every $u_f$ is computed under a "siblings hold still" assumption and is lagged by $\tau_{\rm eff}$. Logarithmic tracking is self-bounded in absolute climb far from the target, since $x\ln(u/x) \le u/e$, so the joint climb of all flows is at most $(k/e)\sum u$ — self-normalised against the targets and therefore **not amplified by the number of flows** — and the joint instantaneous overshoot within the stale window is about $(k/e)\tau_{\rm eff}$. That gives the budget: $k\,\tau_{\rm eff} \lesssim 0.4$, corresponding to roughly 15% instantaneous overshoot. To go faster, the thing to shrink is $\tau_{\rm eff}$ (the measurement window or executor batching), not to raise $k$.
...
The **absolute value** of a tenant CC's rate is of no use to us. It is calibrated against the link, not against a share it has never been told about, and it is free to ratchet — many congestion controls recover toward "the rate I had before the congestion". The minimum combiner $\min(cc,\ level)$ inherits that ratchet whole: each cut also lowers the CC's own recovery target, so a run of cuts walks the flow to near zero and holds it there. At an edge bottleneck shared with another class the process is self-sustaining — RDMA yields its share, TCP expands into it and holds the queue above the marking threshold, and RDMA is marked again before it finishes recovering.
...
Both halves are necessary, and each is taught by a CC that fails without it. **Leaving recovery to the CC's own target ratchets**: a CC that climbs back toward "its pre-congestion self", when every cut drags that target down too, is walked to zero by a run of cuts and pinned there, unable to return while the queue is still being marked. **Leaving recovery to the CC's own step size starves without ratcheting**: a CC that decreases by a factor on each RTT sample and increases by a small fixed step will, in an edge queue held above its delay target by another class, keep receiving decreases that its additive increase can never catch up with, and the flow sits near the floor.
...
**Named configuration modes.** On the demand side, a "share floor" (the accounted share never shrinks, at the cost of killing temporary borrowing during a dip) and a per-class step size (utilisation first, at the cost of ratio drift) are retained as named modes for particular SLA scenarios. Neither is the default.
