...
The executor is the layer closest to the wire. From the sender it receives three things: the flow set's rate ceiling, the $\min(R,U)$ of Section 5.5, written $R$ in this section; the queue $q$ from the most recent feedback; and the "still in the start window" bit. It can read two things on its own: the tenant congestion control's current quota $c$, and whether the flow set has recently lost packets. It keeps one piece of state, the confidence $T\in[0,1]$, and produces one output, the wire rate ceiling $r$.

### 6.1 Clipping

$$r=\operatorname{clip}\big(c,\ (1-T)R,\ R\big).$$

The wire rate ceiling is the congestion control's quota $c$ clipped to the interval $[(1-T)R,\ R]$. The upper end is always $R$, whatever the confidence; the lower end is $(1-T)R$ and opens linearly with confidence. At zero confidence the interval collapses to a point and the wire runs at $R$. At full confidence the interval is $[0,R]$: the congestion control is entirely free below $R$ and still cannot exceed it. $r$ is a ceiling, not a command: an application with nothing to send sends nothing, and the lower bound never pumps traffic on its behalf.

The quota $c$ is defined per class. For RDMA, $c$ is the rate the congestion control maintains, per queue pair; the executor divides the flow set's $R$ equally among its queue pairs and applies the clip per queue pair. For TCP, $c$ is the sum of the congestion windows of the flow set's connections divided by the smallest round-trip time they have seen, capped at line rate, and the clip is applied per flow set. The smallest round-trip time is used rather than the current one because the current round-trip time includes the time packets wait in our shaper, so a value computed from it merely restates the flow's present speed; the value computed from the smallest round-trip time is "how fast TCP would send if nothing were queued on the path", the congestion control's own opinion. The blind spot of this definition is described in Section 8.1.

The clip form gives two properties at once (Properties 1 and 2 in the theory document). The upper end does not move with confidence, so confidence cannot buy bandwidth and no tenant ever gets more than $R$. Inside the interval $c$ is untouched, so the tenant's congestion control sees the network responding to its own rate and its control loop stays intact.

### 6.2 Updating the confidence

Confidence is updated on the executor's own tick $\Delta t$, much shorter than $\tau_r$ (Section 7) and independent of the feedback period; the queue $q$ used is the value carried by the most recent feedback. On every tick the rise condition is evaluated first; if it holds, confidence rises, otherwise it falls.

**The rise condition has three parts, all of which must hold.**

1. The flow set has lost packets within the last observation window. For TCP this means the protocol stack's retransmission counter increased; for RDMA, the device reported a loss event.
2. The quota is below the ceiling by more than the delivery tolerance: $c<(1-\delta)R$. A smaller gap is measurement jitter and does not count as backing off.
3. The queue is zero: $q=0$. A queue means the flow is over-taking, and that is handled first.

Two exceptions make the first part count as false: the flow set is still in its start window (Section 5.4, second rule), or the TCP shaper itself dropped recently and is still in its quiet period (end of this section).

**When all three hold, confidence rises:**

$$T\leftarrow T+\frac{\Delta t}{\tau_r}\,(1-T).$$

Going from zero to near one takes on the order of $\tau_r$.

**Otherwise, confidence falls:**

$$T\leftarrow T-\max\!\Big(\frac{q}{D},\ \frac{\Delta t}{\tau_d}\Big)\,T .$$

The larger of two terms applies. The first falls with the depth of the queue: confidence drops by as much as the flow is over-taking, clearing in a single tick when the queue is full. The second falls with time: on every tick without fresh evidence, confidence is multiplied by a number slightly below one, decaying smoothly toward zero with time constant $\tau_d$.

**The TCP shaper's quiet period.** The TCP shaper drops when its backlog exceeds a cap; that is the safeguard against a congestion control that never yields pushing the backlog to infinity. For one quiet period after such a drop, the flow set's retransmissions do not count toward the first part; the quiet period must cover the retransmission our drop provokes plus the time it stays inside the observation window (Section 7). The RDMA executor only makes queue pairs wait and never drops, so it has no quiet period.

### 6.3 Why it is defined this way

**Confidence has exactly one reason to rise: a bottleneck exists that the receiver's ledger cannot see.** The ceiling $R$ is feasible at the receiver port by construction (theory document, Property 1), so enforcing it can never overrun that port; handing the rate decision to the congestion control makes sense only when the bottleneck is not at that port.

**The test asks about facts of the network, not the state of the congestion control.** A congestion control backing off prematurely at a bottleneck we do control and one facing a genuine hidden bottleneck look identical: the quota is below the ceiling and no longer rising in both cases. The congestion control's state cannot separate them, and a test built on it would raise confidence exactly when the tenant is giving its share away for nothing, letting it give away more. Loss is a fact of the network itself: whatever runs at the ends, a genuinely congested path drops packets, and our own shaper does not (the TCP shaper only delays sending, the RDMA rate limiter only makes queue pairs wait). A recent retransmission in a flow set is therefore direct evidence that the bottleneck limiting it is not the one we manage. That is where the first part comes from; the second part excludes measurement jitter; the third gives over-taking priority over yielding.

**Confidence must expire on its own.** When a bottleneck disappears, no new fact appears anywhere in the network; the only change is that losses stop. And a flow with high confidence can never trigger the queue term of the fall: its upper end is always $R$, it never gets more than its entitlement, and its queue stays at zero. Without the time term, confidence would have a way up and no way down. After one brief hidden bottleneck, this flow's lower-bound protection would stay switched off indefinitely; if its congestion control then backed off under congestion we do control, nobody would push it back to its share, the share would be lent away, the congestion would persist and it could not recover. That is exactly the failure this design exists to eliminate.

**Two costs.** First, a bottleneck that persists but is no longer dropping, because the congestion control has settled below it, is re-probed periodically: confidence expires, the lower bound rises slowly, the flow is pushed back to the bottleneck, loss reappears, and confidence climbs back within $\tau_r$. This is unavoidable, because to the executor "the bottleneck is still there" and "the bottleneck is long gone" look identical, and only pushing up to see whether loss returns tells them apart; with $\tau_d$ several times larger than $\tau_r$ the re-probes are sparse. Second, a congestion control driven by ECN or delay builds a queue at a genuine hidden bottleneck first, and we yield only once loss occurs; treating delay as evidence too is feasible, but it would require adding delay to the premises of Section 1, which this design does not do.

### 6.4 Boundary conditions and initial values

- **Initial values.** A new flow set's confidence is zero. While it has never lost a packet, the first part counts as false; while its quota $c$ cannot be read yet, the second part counts as false.
...
**Congestion where the receiver cannot see it.** When the port is not full and the ledger is correct, only the congestion control knows the path is congested. The design's answer is to hand the rate decision to the congestion control (Section 6.2), and fairness over that stretch is decided by the congestion controls on both sides. The receiver will treat the throttled flow set as a lender and lend its spare capacity to its siblings, who may be crossing the same congested path; the behaviour of the lending rule over such a stretch has not been validated.
