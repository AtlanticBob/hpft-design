...
起步聚合是 $N$ 倍。多对 RDMA 的 runner 要么错开启动、要么加 `--rate_limit`。
发送端代理在每条预算里带"还在起步窗口"一位（RDMA 批量条目第四个字的第 16 位，
TCP 配置 flags 的第 30 位），执行面在窗口里不把丢包当置信度证据（§5.4 第二条）：
不然新流集合自己那一脚撞出的 NACK 会让 slow restart 把 `cc_rate` 切到零、又让
置信度在几百毫秒内升起来，线上就跟着零走而不是跟着围栏。
