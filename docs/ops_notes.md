...
**改过 PCC 设备码之后要重启四台的 RP。** `deploy_check --deploy` 只重编不重启，跑着的还是旧设备码；`bash /opt/hpft/rp_service.sh start`。重启会清空设备内存里的全部流集合预算与 QP 绑定，发送端代理靠 FIFO 的 inode 变化察觉并重发整表。`validation/run/quick.sh` 每场都重启 RP，`run.sh` 不重启（一轮十几场之间不需要）。
...
设计 v4 §5.4（2026-09-02 定）：新流集合的**许可速率**从端口预留余量 $R_0=hC$ 起步
（本平台 16 G），执行面在第一份预算到达之前也按 $R_0$ 放行；RDMA 执行面不知道
未知 QP 属于谁，按 $R_0/$注册表 `rdma_qps_per_flowset`（4）给每个 QP 4 G（邮箱
`0xccf`，由 tx_agent_e 算出下发），所以一个 4 QP 的流集合起步就是 16 G，$N$ 条同时
起步聚合是 $N$ 倍。取 VM 上限的 $h$ 倍（4 G）试过，RDMA 加入慢 0.4 s、TCP 没受益，不取。
**同一台主机上多条流同时起步时，TCP 新流会晚 0.8 s 才真正发**：原因在发送端 DPU
的 OVS 慢路径——新流的头几个包要上送 vswitchd 装流表，RDMA 新流一起步就是每秒
百万级的包，把上送队列挤满（`ovs-dpctl show` 的 `lost` 在加入那 100 ms 里 +13283），
TCP 新连接的初始包一起被丢、进重传超时。这不是控制设计的事，是数据面慢路径的容量。
注册表里已没有 `r_cap_floor_bps` 和 `rdma_unknown_rate_bps`；`r_floor_bps`（1 G）
是许可速率的绝对地板，含义是执行面能正确整形的最低速率，不是起步值。
...
**"杀 tx_agent" 不等于"解除限速"。** RP 在设备内存里保留每个流集合上一次的预算，tx 停了它继续按旧预算整形。真要解除：先停 tx，再 `echo "0xb47f0001 <集合号> 0" > /tmp/rp_fifo`（预算 0 删除流集合），`0xded` 确认；或直接重启 RP（清空全部）。**对偶事实**：凡依赖"停 tx 冻结预算"的编排，必须先让 tx 对全部目标流集合推过预算再停。
...
要造一个"接收端账本管不到的瓶颈"，用交换机一分为二的核心链路（`交换机一分为二` 一节）；早先用过的办法是把接收 VF 的 OVS 丢包 meter 调低而注册表不改。两个量法上的事实：**接收端 vport 计数器读的是 meter 丢包之后的量**，账本只看得见活下来的那部分；**OVS meter 的 band 丢包字节计数在 hw-offload 下恒为 0**，别用 `meter-stats` 量丢包，用交换机计数器、NAK 计数或速率差。

**不要拿没接好的 VF 做实验。** 一个 VF 的流集合在账本里有应得而发送端代理没有它的许可速率时（注册表缺表项、解析器登不上对端、流集合没有集合号），它的 QP 全按"未知流集合"的放行额度跑，实际速率可以是许可速率的几倍，而每一层都报健康。校验用执行面 `0xded` 回读的在册 QP 数，或代理日志里的 `pace-escape`。
...
执行面的设计是每流集合一个令牌桶（design_v4 第 6 节），实现与旋钮在 `hpft-implementation/docs/IMPLEMENTATION.md` 第 3、5 节，租户 CC（DCQCN、Swift）的唯一实现与定稿数字在 `hpft-implementation/tools/dpu/pcc/README.md`。这里只留平台层面的事实。

**限速与拥塞控制是两件事。** 硬件整形器的速率由执行面按事件写入（每个 QP 一个），租户 CC 的状态机逐 QP 在同一段代码里跑；执行面只读 CC 的速率，不改它。DOCA PCC 交给算法的每流上下文不是严格一 QP 一份（两个 VF 的两个 QP 会轮流出现在同一份里，QP 销毁后不清零），所以 QP 记录以 (vhca_id, QPN) 为键，上下文只作缓存。

**执行面是多线程的。** `doca_pcc` 默认在 DPA 上起 48 个事件处理线程加 1 个通信线程（`PCC_RP_THREADS_NUM_DEFAULT_VALUE`，硬件线程号 176 到 240），事件按流散到各线程，用户算法在这些线程上并发运行；全局状态（QP 记录、流集合、绑定表）是共享内存加 `volatile`，没有锁，靠每毫秒的重申与重算修复丢失的更新。

**QP 归属靠主机侧的解析器。** `qpn_resolver` 对每个对端 `rdma res show qp` 配对，只取 RTS/RTR 态的 QP；它不在位时每个 QP 都按未知流集合的额度跑而每一层都报健康，它是发送端必备组件。
...
验证：`0xdef 0` 回读的 RTT 事件计数（第 4 个字）应随 TX 事件增长（实测应答率 99.98%）。
overlay 封装不妨碍 CCMAD，固件流量走端口层，**tcpdump 抓不到不代表没发**。

**执行面从命令到线上的时延约 6 ms**（TCP 侧同口径 8 ms），比 `doca_pcc_mailbox_send` 这个阻塞调用自己返回还早：那 13 到 22 ms 是主机侧阻塞的时长，不在控制路径上，只限制刷新频率，看到收敛长尾不要往这里记。

**诊断邮箱**：`0xdef 0`（事件与绑定计数、算法、臂）、`0xded <槽>`（流集合：号、水位、已整形之和、CC 之和、取令牌 QP 的 CC 之和与个数、在册数）、`0xdee <槽>`（QP：QPN、流集合、CC 速率、已整形、往返、CNP 数、vhca_id）、`0xdf3 0`（绑定诊断）。回读走同一个 FIFO，一次占邮箱 13 到 22 ms，`validation/run/run.sh` 每流集合每秒读一次作为执行面自述。
...
**数据面随之从星型改成静态全网状（2026-09-04）。** 星型 overlay 以接收端为中心，任何两台发送端之间的流量都要经中心的 DPU 中转，所以只能有一个接收端，而且 B 侧第二个接收端的流量会把核心走两遍。网状的做法是每台 DPU 到其它三台各一条隧道，转发不靠 MAC 学习和泛洪，全部由注册表生成显式规则（`overlay.sh`，规则带 cookie `0x4d45`）：本地 VF 发往远端 VF 的包按目的 IP 进对应隧道；隧道进来的包按目的 IP 打 meter 后直接送到本地 VF 的 representor；隧道进来的其它一切丢弃。凡从隧道进来的都不会再从隧道出去，环路在规则层面不可能出现；发往远端的包从不泛洪，MAC 永远不会学到错的口上。主机上的 ARP 照常动态解析（ARP 也按目的地址单播送到拥有它的那台）。两个坑，都因为 NORMAL 不再处理 VF 的出向流量：一是本地 VF 的 MAC 网桥学不到，入向若仍用 NORMAL 送达就会向所有口泛洪、这条 megaflow 硬件不收，实测 0.8 G——所以 `vf_caps.sh` 与 runner 的 meter 规则改成显式输出到 representor；二是显式规则不看 MAC，megaflow 键里就没有 MAC，接收端代理认不出流集合——所以网状规则里有一条"隧道进来的帧源或目的 MAC 为零则丢弃"，分类器为判定它必须看两个 MAC，键里就带上了。改完后 sgpu03→sgpu04 不再经过 sgpu02（sgpu02 隧道收到 0 字节），跨核心 TCP 46.9 G、RDMA 45.6 G，V1 回归五条全过、24 个流集合与改网状之前逐条一致（合计 183.8 G，偏差 −0.3% 以内）。旧的星型保留为 `overlay.sh --star --hub <host>`。

**四台都能收发（2026-09-04 起，`roles.sh all`）。** 每台 DPU 同时跑接收端代理、发送端代理、vport meter 和 RP，每台主机跑 pace shim 和 QP 解析器；两套代理在一台 DPU 上互不相扰（接收端代理听活性口和速率导出口，发送端代理听遥测口，vport meter 共用）。runner 已改为调它，谁收谁发只看打流表。为此改了三处：注册表 `control.pace_shim` 补上 sgpu02（`rate_export` 早已退役，不用补）；发送端代理的活性报告发给所有其它节点；QP 解析器接受多个对端。V1 回归五条全过，24 个流集合合计 183.4 G，偏差 −0.7% 以内。

**RDMA 流标签与目的端有关，每个接收主机都要实测一次。** 执行面靠硬件流标签认流对。以前只知道它与源网卡无关（sgpu03/sgpu04 打 sgpu02 的标签和 sgpu01 的相同），这次发现它随目的端变：vf0→vf0 打 sgpu02 是 `0x74249a41`，打 sgpu04 是 `0x1e3dac89`；同一目的端从 sgpu01 和 sgpu03 打过去仍相同。注册表里原先只有打 sgpu02 的 48 对，缺标签的流对症状很典型：发送端代理写不了预算，执行面按"未知 QP 额度"放行，4 QP 的流恰好卡在 16 G（4 G × 4），而许可速率本身正常上探。测法：`tools/lab-infra/flowtag_probe.sh <源主机> <目的主机>` 对每个直连对跑一段两 QP 的短流，同时向源端 DPU 的 RP 邮箱写控制行 `0xdea 0`（必须两个词，单个词会被邮箱进程当空行丢掉），从 `/tmp/pcc_rp.log` 的 `HPFT_RSP` 读回"最近一个未知流标签"；`registry_flowtags.py` 把结果写进注册表。已加 sgpu04、sgpu03、sgpu01 作为目的端的 vf0–vf7 直连对；交叉对（vfS→vfD，S≠D）只有打 sgpu02 的，验证套件也只用直连对。发送端代理对没有自己那一行的流对按（源 VF 序号，目的 vnic）兜底查表。

**QP 解析器必须以登录用户跑、四台之间用户密钥齐全。** 它对每个对端 `rdma res show qp` 做配对，只取 RTS/RTR 态的 QP（上一场残留的 ERR/RESET 态 QP 的号会被新 QP 复用），登不上对端就一行都不发，那台机器的每个 QP 都按未知流集合的额度跑而每层服务都报健康；`deploy_check.sh` 检查用户 ssh 密钥。执行面的 QP 记录以 (vhca_id, QPN) 为键，`0xded` 回读的在册数等于每流集合的 QP 数才算绑齐。
