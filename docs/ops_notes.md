...
**执行面从命令到线上的时延约 6 ms**（TCP 侧同口径 8 ms），比 `doca_pcc_mailbox_send` 这个阻塞调用自己返回还早：那十几毫秒是主机侧阻塞的时长，不在控制路径上，只限制刷新频率，看到收敛长尾不要往这里记。

**诊断邮箱**：`0xdef 0`（事件与绑定计数、算法、臂）、`0xded <槽>`（流集合：号、水位、已整形之和、CC 之和、取令牌 QP 的 CC 之和与个数、在册数）、`0xdee <槽>`（QP：QPN、流集合、CC 速率、已整形、往返、CNP 数、vhca_id）、`0xdf3 0`（绑定诊断）。回读走同一个 FIFO，一次占一趟邮箱（约 16 ms），`validation/run/run.sh` 每流集合每秒读一次作为执行面自述。
