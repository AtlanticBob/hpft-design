...
要验证信任度就得造一个"接收端账本管不到的瓶颈"。现有星型拓扑里唯一会拥塞的链路就是接收端口，而账本建模的正是它，所以自然的隐形瓶颈不存在，只能人工造。把接收 VF 的 OVS 丢包 meter 调低、注册表不改，是最省事的一种。探路量到的四件事：
...
- **崩溃是真的。** 19.4 G 灌进 8 G 的口子，应用 goodput 掉到 4.9–5.2 G——**比这个口子本身能过的还少**，而接收端账本全程报队列 0、入场额 25 G。这正是信任度要治的病，二十秒就能复现。
...
**信任度是租约**（design v4 §6.2/6.3，2026-09-02 起）：抬升三条件（队列为零、
`cc_rate` 低于 level 2% 以上、最近 100 ms 有 NACK）成立的每个 1 ms epoch 涨
$(1-T)\cdot$步长，不成立的每个 epoch 按 $T\cdot\max(q/D,\ \text{到期步长})$
降。两个步长都是 fxp16/epoch 的邮箱旋钮：`0xcce <步长> 7` 是抬升（默认 66 =
$\tau_r$ 1 s；0 = 信任冻结在零，V8 臂 B），`0xcce <步长> 10` 是到期（默认 13 =
$\tau_d$ 5 s；0 = 关掉到期，即 2026-09-02 之前只有队列一条下降通路的锁存规则，
V9 臂 B）。到期项必须在的原因：瓶颈消失不产生网络事实，被信任的流又造不出多拿
（上界恒为 level），没有它信任就没有退出条件——实测锁存规则下瓶颈撤掉后 $T$ 的
衰减时间常数约 278 s，下界保护停用十几分钟。
